...
const SCRATCH_DIR =
  "C:\\Users\\gulac\\AppData\\Local\\Temp\\claude\\C--Users-gulac-OneDrive-Bureau\\de5ca641-0a30-4d29-97ba-d4ad6fc9aaa7\\scratchpad";
...
// rather than imported because this script runs outside Next's module
// resolution (no "@/..." path aliases here).
async function uploadDemoFile(organizationId: string, moduleId: string, filename: string, buffer: Buffer, contentType: string) {
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    throw new Error("BLOB_READ_WRITE_TOKEN manquant dans .env — impossible d'uploader les fichiers de démo.");
  }
...
async function main() {
  ...
  // ============================================================
  // 1. Fill in the missing roles — prisma/seed.ts already covers
  ...
  if (!modVideo1.fileUrl) {
    const buf = fs.readFileSync(path.join(SCRATCH_DIR, "video1.mp4"));
  ...
  if (!modVideo2.fileUrl) {
    const buf = fs.readFileSync(path.join(SCRATCH_DIR, "video2.mp4"));
  ...
  console.log("Comptes (mot de passe pour tous : " + DEMO_PASSWORD + ") :");
  console.log("  ADMIN_OF        marie@formations-nova.fr");
  console.log("  ADMIN_MANAGER   nadia.rahmani@formations-nova.fr");
  console.log("  SALES           julien.petit@formations-nova.fr");
  console.log("  TRAINER         claire.bonnet@formations-nova.fr");
  console.log("  TRAINER         thomas.marchand@formations-nova.fr");
  console.log("  DPO_EXTERNAL    isabelle.roche@dpo-externe.fr");
  console.log("  LEARNER         jean.dupuis@atlas-conseil.fr   (formation en continu, en retard)");
  console.log("  LEARNER         sophie.martin@clientdemo.fr    (rien commencé)");
  console.log("  LEARNER         karim.benali@clientdemo.fr     (formation en cours)");
  console.log("  LEARNER         lea.fontaine@clientdemo.fr     (formation terminée, attestation prête)");
  console.log("  LEARNER         marc.dubois@clientdemo.fr      (délai dépassé, alerte visible)");
}
